...
app.use(
  cors({
    origin: process.env.FRONTEND_URL || 'https://test-d-yvso.vercel.app',
...
app.post('/register', async (req, res) => {
  const { username, password, telephone, privilege } = req.body;
  ...
  try {
    ...
    if (existingPhone.length === 0) {
      return res
        .status(409)
        .json({ message: "Le numéro de téléphone n'appartient à aucun membre." });
    }
    ...
    const queryInsertUser =
      'INSERT INTO users (username, password, telephone, privilege) VALUES (?, ?, ?, ?)';
...
app.post('/membre', upload.single('photo'), (req, res) => {
...
// Route pour récupérer un utilisateur par son ID
app.get('/usersManage/:id', (req, res) => {
  const membreId = req.params.id;
...
app.put('/membre/:id', upload.single('photo'), (req, res) => {
  const membreId = req.params.id;
  const { nom, prenoms, surnom, date_naissance, situation, enfants, profession, taille, pointure, adresse, telephone, facebook, whatsapp } = req.body;
...
app.post('/create_transactions', (req, res) => {
...
app.put('/transactions/:date', async (req, res) => {
  ...
  app.delete('/transactions/:formattedDate', (req, res) => {
...
// Ajouter un membre
app.post('/membres_bureau_add', (req, res) => {
...
// Modifier un membre
app.put('/membres/:telephone', (req, res) => {
...
// Supprimer un membre
// app.delete('/membres/:telephone', (req, res) => {
//     const { telephone } = req.params;
...
app.use((req, res) => {
  res.status(404).json({ error: 'Route not found...OMG' });
});

app.use((req, res) => {
  res.status(500).json({ error: 'Server error....OMG' })
})
